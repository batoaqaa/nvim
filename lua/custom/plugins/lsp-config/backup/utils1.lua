...
function M.ToggleTerminal(command, direction)
  ...
  local termConfig = {
    ...
    on_open = function(t)
      ...
      -- vim.keymap.set("c", "q!", [[<cmd>PioTermQuit]], { desc = "PioTermQuit", noremap = true, buffer = t.bufnr })
      vim.keymap.set("c", "q!", function()
        PioTermQuit()
      end, { desc = "PioTermQuit", noremap = true, buffer = t.bufnr })
